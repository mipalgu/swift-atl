...
struct ATLDebugTests {
    ...
    func testContextHelperTokenisation() async throws {
        ...
        // Check key tokens
        #expect(tokens.contains { if case .keyword("helper") = $0.type { return true }; return false })
        #expect(tokens.contains { if case .keyword("context") = $0.type { return true }; return false })
        #expect(tokens.contains { if case .identifier("String") = $0.type { return true }; return false })
        #expect(tokens.contains { if case .keyword("def") = $0.type { return true }; return false })
    }
    ...
    func testHelperFunctionsExcerpt() async throws {
        ...
        do {
            ...
            for helper in module.helpers.values {
                print("   - \(helper.name): \(helper.parameters.count) parameters, returns \(helper.returnType)")
            }
        } catch {
...
private class TestATLLexer {
    ...
    func tokenize() throws -> [ATLToken] {
        ...
        while position < content.endIndex {
            ...
            let twoChar = String(content[position..<min(content.index(position, offsetBy: 2), content.endIndex)])
            if Self.operators.contains(twoChar) {
                tokens.append(ATLToken(type: .operator(twoChar), value: twoChar, line: startLine, column: startColumn))
                advance()
                advance()
                continue
            }
            ...
            if Self.operators.contains(singleChar) {
                tokens.append(ATLToken(type: .operator(singleChar), value: singleChar, line: startLine, column: startColumn))
                advance()
                continue
            }
            
            if Self.punctuation.contains(singleChar) {
                tokens.append(ATLToken(type: .punctuation(singleChar), value: singleChar, line: startLine, column: startColumn))
                advance()
                continue
            }
    ...
    private func parseIdentifier(startLine: Int, startColumn: Int) -> ATLToken {
        ...
        if Self.keywords.contains(value) {
            return ATLToken(type: .keyword(value), value: value, line: startLine, column: startColumn)
        } else {
            return ATLToken(type: .identifier(value), value: value, line: startLine, column: startColumn)
        }
    }
    ...
    private func parseNumber(startLine: Int, startColumn: Int) -> ATLToken {
        ...
        while position < content.endIndex && (content[position].isNumber || content[position] == ".") {
            advance()
        }
        ...
        if let intValue = Int(value) {
            return ATLToken(type: .integerLiteral(intValue), value: value, line: startLine, column: startColumn)
        } else {
            return ATLToken(type: .identifier(value), value: value, line: startLine, column: startColumn)
        }
    }
    ...
    private func parseString(startLine: Int, startColumn: Int) throws -> ATLToken {
        ...
        return ATLToken(type: .stringLiteral(value), value: value, line: startLine, column: startColumn)
    }
